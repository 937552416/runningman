...
def _near_enemy( my_position, items, dist, prev, enemies, radius):
    nearest_enemy_position = _nearest_position(dist, enemies, items,
                                                   radius)
    return _get_direction_towards_position(my_position,
...
def available_checking(obs):
    ...
    items, dist, prev = _djikstra(
        board, my_position, bombs, enemies, depth=10)
    if _is_adjacent_enemy(items, dist, enemies) and _maybe_bomb(ammo, blast_strength, items, dist, my_position):
        return 1
    elif _near_wood(my_position, items, dist, prev, 1):
...
def safe_filter(obs):
    ...
    directions = _filter_unsafe_directions(board, my_position,
                                                direction, bombs)
    # return directions[0].value
    if directions:
        return [direct.value for direct in directions]
    else:
        return [0]

def find_safe_directions(obs):
    def convert_bombs(bomb_map):
        '''Flatten outs the bomb array'''
        ret = []
        locations = np.where(bomb_map > 0)
    ...
    items, dist, prev = _djikstra(
        board, my_position, bombs, enemies, depth=10)
    unsafe_directions = _directions_in_range_of_bomb(
        board, my_position, bombs, dist)
    if unsafe_directions:
        directions = _find_safe_directions(
...
def agent_filter(obs,_recently_visited_positions,_prev_direction):
    _recently_visited_length = 6
    def convert_bombs(bomb_map):
        '''Flatten outs the bomb array'''
        ret = []
        locations = np.where(bomb_map > 0)
    ...
    if direction is not None and (_prev_direction != direction or
                                  random.random() < .5):
        _prev_direction = direction
        return direction.value

    # Move towards a good item if there is one within two reachable spaces.
    direction = _near_good_powerup(my_position, items, dist, prev, 2)
    ...
    valid_directions = _filter_invalid_directions(
        board, my_position, directions, enemies)
    directions = _filter_unsafe_directions(board, my_position,
                                                valid_directions, bombs)
    directions = _filter_recently_visited(
        directions, my_position, _recently_visited_positions)
    if len(directions) > 1:
    ...
    if directions:
        ...
        return random.choice(directions).value,_prev_direction,_recently_visited_positions